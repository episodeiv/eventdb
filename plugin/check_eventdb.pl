...
# Count matches
push @sql, sprintf('SELECT COUNT(id) as count, MAX(id) as last, GROUP_CONCAT(message SEPARATOR \'\n\') as output from %1$s where id>%2$d', $opt_dbtable, $startfrom);

add_part('host_name', 1, $opt_host, 'host')
	if ($opt_host ne '');

add_part('message', 0, $opt_msg)
	if ($opt_msg ne '');

add_part('priority', 1, $opt_priority)
	if ($opt_priority ne '');

add_part('type', 1, $opt_type)
	if ($opt_type ne '');

add_part('facility', 1, $opt_facility)
	if ($opt_facility ne '');

add_part('created', 1, $opt_maxage, ">=")
	if ($opt_maxage ne '');

add_part('program', 0, $opt_program)
	if ($opt_program ne '');
...
sub myexit {
	...
	if ($opt_url) {
		...
		'<a target="_blank" href="' . $opt_url . '?' . join('&', @urlparams) . '">' . $text . '</a>');
	} else {	
...
sub add_part {
	my ($field, $is_array, $value, $op, $agg, $fieldname) = @_;
