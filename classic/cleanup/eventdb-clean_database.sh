...
# backup
$MYSQL --execute="SELECT * INTO OUTFILE '$BACKUPDIR/$OLDDATE.txt' FIELDS TERMINATED BY '\t' FROM events WHERE datetime < '$OLDDATE' $ACKNOWLEDGED;"

# clean
$MYSQL --execute="DELETE FROM events WHERE datetime < '$OLDDATE' $ACKNOWLEDGED;"
